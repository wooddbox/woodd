...
class pb_backupbuddy_mysqlbuddy {
	...
	private function _calculate_mysql_dir() {
		...
		global $wpdb;
		$result = mysql_query( "SHOW VARIABLES LIKE 'basedir'", $wpdb->dbh );
		if ( $result !== false ) {
	...
	private function _dump_php( $output_directory, $tables ) { //, $base_dump_mode, $additional_excludes ) {
		...
		foreach( $tables as $table_key => $table ) {
			...
			$create_table = mysql_query( "SHOW CREATE TABLE `{$table}`", $wpdb->dbh );
			...
			while ( true === $rows_remain ) {
				...
				$table_query = mysql_query( $query, $wpdb->dbh ) or pb_backupbuddy::status( 'error', 'Error #9001: Unable to read database table `' . $table . '`. Your backup will not include data from this table (you may ignore this warning if you do not need this specific data). This is due to the following error: ' . mysql_error( $wpdb->dbh ) );
				...
				if ( 0 == mysql_num_rows( $table_query ) ) {
					$rows_remain = false;
					break;
				}
	...
	public function import( $sql_file, $old_prefix, $query_start = 0, $ignore_existing = false ) {
		...
		if ( $ignore_existing === false ) {
			if ( $query_start == 0 ) { // Check number of tables already existing with this prefix. Skips this check on substeps of DB import.
				$result = mysql_query( "SHOW TABLES LIKE '" . mysql_real_escape_string( str_replace( '_', '\_', $this->_database_prefix ) ) . "%'", $wpdb->dbh );
	...
	function _import_sql_dump_line( $query, $old_prefix, $ignore_existing ) {
		...
		// Run the query
		// Disabled to prevent from running on EVERY line. Now just running before this. mysql_query("SET NAMES 'utf8'", $wpdb->dbh ); // Force UTF8
